...
ROOT=/home/xlive/sparrow/testSparrow

DEST=$ROOT/out/3.0.2 && rm -rf $DEST
FF=ffmpeg-3.0.2
SOURCE=$ROOT/build/$FF

if [ -d $SOURCE ]; then
  echo "$SOURCE exits!"
  cd $SOURCE
else
  echo " $SOURCE not exit "
fi
...
for version in neon; do  ##only neon
  ...
  echo "gen libm_hard.a ok"

  echo "try to generate libffmpeg.so"
  ...
  echo "NOW WARE IN " `pwd`
  echo "try to strip libffmpeg.so"
